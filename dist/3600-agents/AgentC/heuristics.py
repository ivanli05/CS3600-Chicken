"""
Heuristics and Move Evaluation for AgentC

This module contains strategic evaluation functions for moves and positions,
with enhanced enemy tracking and corner prioritization.
"""
...
class MoveEvaluator:
    ...
    Enhanced with enemy egg tracking and aggressive corner seeking.
    """
    ...
    def quick_evaluate_move(
        ...
        enemy_egg_locations=None
    ) -> float:
        """
        Quick heuristic evaluation of a move for move ordering.
        
        Enhanced with enemy egg avoidance and aggressive corner seeking.
        """
        ...
        # 0. MASSIVE penalty for blocked locations (enemy eggs/barriers)
        if blocked_locations is not None and new_loc in blocked_locations:
            score -= 50000.0
        
        # Also check if board says it's blocked
        if board.is_cell_blocked(new_loc):
            score -= 50000.0

        # 0.5. CRITICAL: Avoid squares where enemy has laid eggs
        # Enemy eggs block us from laying eggs there, so avoid them
        if enemy_egg_locations is not None and new_loc in enemy_egg_locations:
            score -= 30000.0  # Strong penalty - can't lay eggs here
        
        # Also check board's enemy eggs
        if hasattr(board, 'eggs_enemy') and new_loc in board.eggs_enemy:
            score -= 30000.0

        # 1. Egg moves are HIGHLY valuable (direct scoring) - MAXIMUM EMPHASIS!
        if move_type == MoveType.EGG:
            score += 400.0  # Increased base value for eggs
            
            # Check if this chicken can actually lay eggs here (parity check)
            can_lay = board.chicken_player.can_lay_egg(new_loc)
            if not can_lay:
                score -= 100000.0  # Can't lay eggs here - massive penalty
            else:
                # Corner eggs are EXTREMELY valuable (3x egg value = 3 eggs!)
                if self._is_corner(new_loc):
                    score += 800.0  # MASSIVE bonus for corner eggs (3x value!)
                    # Extra bonus if we're close to corner
                    if abs(new_loc[0] - my_loc[0]) + abs(new_loc[1] - my_loc[1]) <= 1:
                        score += 200.0  # Very close to corner
                
                # Bonus for laying eggs in new/unexplored areas
                if visited_squares is None or new_loc not in visited_squares:
                    score += 200.0  # Strong bonus for eggs in new areas
                
                # Bonus for laying eggs far from existing eggs (spread out)
                if hasattr(board, 'eggs_player') and board.eggs_player:
                    ...
                    # Bonus for spreading eggs out
                    if min_dist_to_existing_egg >= 4:
                        score += 80.0
                    elif min_dist_to_existing_egg >= 3:
                        score += 40.0
                    elif min_dist_to_existing_egg <= 1:
                        score -= 50.0  # Penalty for clustering too close
                
                # AVOID laying eggs near enemy eggs (they block us)
                if hasattr(board, 'eggs_enemy') and board.eggs_enemy:
                    min_dist_to_enemy_egg = min(
                        abs(new_loc[0] - egg[0]) + abs(new_loc[1] - egg[1])
                        for egg in board.eggs_enemy
                    )
                    if min_dist_to_enemy_egg <= 1:
                        score -= 100.0  # Penalty for being too close to enemy eggs
        ...
        if move_type == MoveType.PLAIN:
            ...
            # Bonus for plain moves toward accessible corners
            ...
            if accessible_corners:
                ...
                # Bonus for moving toward corners
                if min_dist_to_corner <= 2:
                    score += 100.0  # Very close to corner
                elif min_dist_to_corner <= 4:
                    score += 50.0  # Getting closer
        ...
        # 2. Turd moves for strategic blocking
            ...
            if board.chicken_player.get_turds_left() > 0:
                ...
                # Blocking paths to valuable squares (especially corners)
        ...
        if trapdoor_tracker:
            ...
            if is_known_trapdoor:
                score -= 1000000.0  # ABSOLUTE penalty
            else:
                # Heavy penalty for likely trapdoors
                if danger > 0.2:
                    score -= danger * 200000.0
                elif danger > 0.1:
            ...
            # Avoid staying around known trapdoors
            if trapdoor_tracker.known_trapdoors:
                ...
                # Bonus for moving far away from known trapdoors
                if min_dist_to_trapdoor >= 5:
                    score += 250.0
                elif min_dist_to_trapdoor >= 4:
                    score += 120.0
                
                # Extra exploration bonus when moving away from trapdoors
        ...
        # 6. STRONG anti-repetition: penalize revisiting squares
        ...
        if visited_squares is not None:
            if new_loc in visited_squares:
                score -= 500.0  # Strong penalty for revisiting
            else:
                # LARGE bonus for exploring new squares
                score += 200.0  # Increased bonus for exploration
                
                # Extra bonus for exploring different regions
        ...
        # 7. AGGRESSIVE corner seeking - corners give 3x egg value!
        accessible_corners = self._get_accessible_corners(board)
        if accessible_corners:
            min_dist_to_corner = min(
                abs(new_loc[0] - corner[0]) + abs(new_loc[1] - corner[1])
            ...
            # Strong bonus for moving toward accessible corners
            if visited_squares is None or new_loc not in visited_squares:
                corner_bonus = max(0, (14 - min_dist_to_corner) * 8.0)  # Increased multiplier
                score += corner_bonus
    ...
    def _evaluate_position_quality(self, board: board_module.Board) -> float:
        """Evaluate positional factors like territory control and corner control"""
        score = 0.0
        ...
        # Corner control - check if we have eggs in corners
        corners = [
            (0, 0), (0, self.map_size - 1),
            (self.map_size - 1, 0), (self.map_size - 1, self.map_size - 1)
        ]
        my_corner_eggs = sum(1 for corner in corners if corner in board.eggs_player)
        enemy_corner_eggs = sum(1 for corner in corners if corner in board.eggs_enemy)
        score += (my_corner_eggs - enemy_corner_eggs) * 300.0  # Corners are 3x value!
    ...
    def _get_valuable_egg_squares(self, board: board_module.Board) -> List[Tuple[int, int]]:
        """Get list of valuable egg-laying positions (especially corners)"""
        valuable = []

        # Corners are MOST valuable (3x egg value!)
        corners = [
            (0, 0), (0, self.map_size - 1),
            (self.map_size - 1, 0), (self.map_size - 1, self.map_size - 1)
        ]
    ...
    def _count_egg_clusters(self, eggs: set) -> int:
        """Count eggs that are adjacent to other eggs"""
        cluster_count = 0
        for egg in eggs:
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
