...
class MoveEvaluator:
    ...
    def quick_evaluate_move(
        ...
        # The farther we are from the opponent, the less likely we get blocked
        ...
        if dist_to_enemy >= 6:
            ...
            score -= 300.0  # Too close - danger of blocking!
        ...
        if move_type == MoveType.PLAIN:
            ...
            if visited_squares is not None:
                if new_loc in visited_squares:
                    # Apply penalties ONLY if not oscillating
                    if not is_oscillating:
                        # Moderate penalty for revisiting squares with plain moves
                        score -= 400.0 * desperation_factor
                        ...
                        if visit_count > 0:
                            ...
                            score -= visit_count * visit_count * 250.0 * desperation_factor
                else:
