"""
Trapdoor Probability Tracker for AgentB

This module tracks and updates probability distributions for trapdoor locations
using Bayesian inference based on sensor readings (hearing/feeling).
"""
...
class TrapdoorTracker:
    ...
    Maintains two probability grids (one for white squares, one for black)
    and updates them based on sensor feedback (hearing/feeling trapdoors).
    """
    ...
    def _initialize_prior(self, color: str) -> np.ndarray:
        ...
        Trapdoors are weighted toward center:
    ...
    def update_beliefs(
        ...
        """
        Update probability beliefs based on sensor readings.
    ...
    def _bayesian_update(
        ...
        for i in range(self.map_size):
            for j in range(self.map_size):
                ...
                if felt:
                    # We felt it - high probability it's close
                    likelihood[i, j] = p_feel
                elif heard:
                    # We heard but didn't feel - could be diagonal or 2-away
