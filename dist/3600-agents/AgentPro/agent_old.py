...
class PlayerAgent:
    ...
    def __init__(self, board: board_module.Board, time_left: Callable):
        ...
        # Neural network evaluator (with proper feature extraction)
        self.nn_evaluator = None
        self.use_nn_eval = False
        if TORCH_AVAILABLE:
            try:
                self.nn_evaluator = PositionEvaluator()
                loaded = self.nn_evaluator.load_weights()  # Try to load trained weights
                if loaded:
                    self.nn_evaluator.eval()
                    self.use_nn_eval = True
                    print("✓ Neural network loaded successfully")
                else:
                    print("⚠ Neural network not loaded - using heuristics only")
            except Exception as e:
                print(f"⚠ Neural network loading failed: {e}")
                self.nn_evaluator = None

        # Search engine (will use NN if available)
