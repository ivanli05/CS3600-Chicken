...
class MoveEvaluator:
    ...
    def evaluate_position(
        ...
        # --- 5. TURD USAGE PENALTY ---
        # Penalize using turds as a limited resource
        turds_used = 5 - board.chicken_player.get_turds_left()
        enemy_turds_used = 5 - board.chicken_enemy.get_turds_left()
        # Each turd used costs 200 points unless it provides strategic value
        score -= (turds_used - enemy_turds_used) * 200.0
    ...
    def quick_evaluate_move(
        ...
        if move_type == MoveType.EGG:
            ...
            # Turds are a limited resource - penalize using them unless strategic
            # Base penalty discourages early/casual turd usage
            score -= 100.0 
    ...
    def debug_evaluate_position(self, board: board_module.Board, trapdoor_tracker=None, move_name="Current") -> float:
        ...
        turds_used = 5 - board.chicken_player.get_turds_left()
        enemy_turds_used = 5 - board.chicken_enemy.get_turds_left()
        turd_penalty = (turds_used - enemy_turds_used) * 50.0
        if turd_penalty != 0:
