...
class SearchEngine:
    ...
    def _minimax(
        ...
        if depth == 0:
            score = self.evaluator.evaluate_position(board, trapdoor_tracker, visited_squares)
            if not maximizing:
                score = -score
            return score, None
        ...
        # 4. Move Ordering
        ordered_moves = self._order_moves(valid_moves, board, trapdoor_tracker, visited_squares)
    ...
    def _order_moves(self, moves, board, trapdoor_tracker, visited_squares=None):
        """
        Heuristic to sort moves so Alpha-Beta pruning cuts off branches earlier.
        """
        if visited_squares is None:
        ...
        scores = []
        for move in moves:
            score = self.evaluator.quick_evaluate_move(move, board, trapdoor_tracker, visited_squares)
            scores.append((score, move))
